...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            print('grad_norm',i, total_norm)
        
        total_norm = 0
        for p in list(filter(lambda p: p.grad is not None and p.requires_grad == True, model.parameters())):
            #print('1st for loop')
            param_grad = p.grad.data
            param_grad = param_grad.flatten()
            #print(param.size())
            param_grad = torch.square(param_grad)
            param_grad = torch.sum(param_grad)
            total_norm += param_grad
        total_norm = total_norm ** (1. / 2)
        if (i + 1) % args.print_freq == 0 and main_process():
            print('grad_norm',i, total_norm)
        '''
